...
syn keyword	lpcOperator	catch efun in inherit SAVE_F
...
syn keyword	lpcKeywdFunc	new parse_command time_expression
...
" Scalar/Value types.
syn keyword	lpcType		int float string true false bool
" Pointer types.
...
syn keyword	lpcModifier	varargs virtual static

" sensible modifiers
if exists("lpc_pre_v22")
    syn keyword	lpcReserved	nosave protected ref
    syn keyword	lpcModifier	staticstis
else
    syn keyword	lpcError	staticstis
...
" Applies list.
"       system applies
syn keyword     lpcApplies      contained __INIT clean_up create destructor heart_beat id init move_or_destruct reset do_destruct
"       interactive
...
syn keyword     lpc_efuns       contained parse_add_rule parse_add_synonym parse_command parse_dump parse_init parse_my_rules parse_refresh parse_remove parse_sentence pluralize pointerp pow present previous_object printf process_string process_value program_info sscanf
...
syn keyword	lpc_efuns	contained obj_2_string QuickRandArr sscanf tell_user tell_fd SafeGetiUser SafeGetcUser SafeGetmixUser SafeGetiNpc SafeGetcNpc SafeGetmixNpc SafeGetiItem SafeGetcItem NotifySkillLevel state_change GetiUser GetcUser GetmixUser GetiNpc GetcNpc GetiItem GetcItem
syn keyword	lpcApplies	contained SafeSetiUser SafeSetcUser SafeSetmixUser SafeSetiNpc SafeSetcNpc SafeSetmixNpc SafeSetiItem SafeSetcItem SetiUser SetcUser SetmixUser SetiNpc SetcNpc SetiItem SetcItem DirSetiUser DirSetcUser
syn keyword	lpc_efuns	contained escape_from_war leave_team Get_Scene_User_List Get_Scene_User_Count Pet_Status Send_Ext_Chat Set_User_Summon Set_User_Pet Pawn_Summon List_Pawn_Summon Redeem_Summon Set_Block Get_Block Get_Current_Period Get_Map_Weather Set_Map_Weather Send_Map_Weather clone_user get_coordinate Add_Org_Scene Add_Org_Small_Scene Del_Scene Get_Org_Count Read_Mail Max_Grade Min_Grade Avg_Grade Team_Member NotifySkillLevel ModFriend GetFriendStatus GetAllFriend GetAllFriend2 normal_npc_walk fight_npc_walk npc_walk_to Pawn_Item Redeem_Item Redeem_Price List_Pawn_Items TellMagicEffect Get_Urs_Info GetFightTeam GetDownTeam GetAllTeam Items_Full _get_player_count _get_player_id all_player all_skills Get_Npc_Object Get_Item_Object Destroy_Npc Destroy_Item Get_User_Items Get_User_AllItems Transfer_Item GetFightSkill find_uid_by_name Start_Fight Add_Fight End_fight_Init GetFight GetFightValue SetFightValue SetFightValueNoRsync ModFightValue ModFightValueNoRsync tell_fighter do_attack Player_Enter_Place Npc_Enter_Place Baby_Enter_Place Baby_Mod_Shape Npc_Delete_From_Place refresh_user_state kickout_user del_user set_ver Item_Enter_Place Send_Around_Scene Send_Game_Message New_Item New_Npc SetCurrentNpc GetCurrentNpc NewNpc RemoveNpc SetCurrentItem NewItem RemoveItem Item_String SetCurrentUser GetCurrentUser NewUser RemoveUser This_UserNum state_change Items_Operate Refresh_Item GetmixUser SetmixUser tell_user Add_Summon Del_Summon Find_Summon All_Summon Get_User_Ip cal_five get_current_sp bianshen_card refresh_ride All_Ride Del_Ride Add_Ride Ride_Info Set_Ride_Info Add_Fabao set_fabao get_fabao GetCurrentItem GetUserObj add_action all_inventory allocate allocate_mapping bind print_cache_stats cache_stats call_other call_out call_out_info origin_name call_stack capitalize children classp clear_bit clonep clone_object command commands cp ctime deep_inherit_list set_debug_level deep_inventory destruct dumpallobj ed error environment exec explode file_name file_size filter find_call_out find_living find_object find_player get_char get_config get_dir implode inherits inherits shallow_inherit_list input_to interactive intp functionp keys values living livings lower_case lpc_info malloc_status map_delete mapp map master match_path member_array message mkdir move_object txe_status notify_fail objectp opcprof origin pointerp present previous_object printf process_string process_value get_host_name get_idle query_load_average get_snooping get_snoop get_verb random read_bytes read_file receive reg_assoc regexp remove_call_out rename replace_string filtedWord replace_string_reg restore_object restore_object_txdb restore_variable rm rmdir save_object save_variable say set_eval_limit set_txos set_bit set_heart_beat get_heart_beat set_hide shout shutdown sizeof snoop sprintf cmd_printf scene_printf around_printf fight_printf fight_printf2 shout_printf stat substr strsrch strcmp stringp bufferp swap tail tell_object tell_room test_bit next_bit this_object this_player set_this_player throw time to_float to_int typeof undefinedp uptime userp users virtualp write write_bytes write_file dump_file_descriptors reclaim_objects memory_info reload_object query_shadowing set_reset floatp flush_messages first_inventory next_inventory save_func run_func Remove_VItems Set_VItems Update_Player_Prop Send_Baby_Att obj_hash test_crypt send_bin_msg allocate_buffer read_buffer write_buffer pk_push_int8 pk_push_int16 pk_push_int32 pk_pop_int8 pk_pop_int16 pk_pop_int32 send_buffer send_buffer_fd md5_hexdigest tell_fd Set_Map_Daytime get_pawn_item_info check_fight_status reset_small_scene update_item_ptr lpc_call_script change_leader DirGetiUser DirGetcUser DECLARE_STATIC_VAR DECLARE_RUN_VAR DECLARE_PERM_VAR
" Nodule: Constants {{{1

" LPC Constants.
" like keywords, constants are always highlighted, be careful to choose only
" the constants we used to add to this list.
syn keyword     lpcConstant     __ARCH__ __COMPILER__ __DIR__ __FILE__ __LINE__ __OPTIMIZATION__ __PORT__ __VERSION__
...
syn keyword	lpcTodo		contained TODO FIXME XXX LATER
